...
export async function listCreators(limit = 12): Promise<Creator[]> {
  // 🔧 Vercel KV doesn’t have zrevrange — use zrange with { rev: true }
  ...
  return rows.filter((r): r is Creator => !!r && Object.keys(r).length > 0)
}
...
export async function getRecentRatings(creatorId: string, limit = 10) {
  const items = await kv.lrange<string[]>(RATING_RECENT(creatorId), 0, Math.max(0, limit - 1))
