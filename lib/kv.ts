...
/* ---- in-memory shim (for local / missing envs) ---- */
...
/* ---- minimal interface we use (loosened return types to match Redis typings) ---- */
type KVLike = {
  ...
  set(
    ...
    val: string,
    opts?: { ex?: number }
  ): Promise<any>; // Upstash returns Promise<"OK">, loosen to any
...
export const kv: KVLike = KV_ENABLED
  ? (new Redis({ url: URL, token: TOKEN }) as unknown as KVLike)
  : {
