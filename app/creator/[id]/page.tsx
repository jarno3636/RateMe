...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    title,
    openGraph: { title, url: `${SITE}/creator/${id}`, images: [{ url: `${SITE}/miniapp-card.png`, width: 1200, height: 630 }] },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-8">
      ...
      <section className="flex items-center gap-4">
        <img src={creator.avatarUrl || '/icon-192.png'} alt="" className="h-16 w-16 rounded-full ring-2 ring-white/10" />
        <div>
      ...
      <OnchainSections creatorAddress={creator.address as any} />
                ...
                <li key={r.createdAt} className="rounded-lg border border-white/10 bg-white/5 p-2 text-sm">
                  <div className="flex items-center gap-2">
                    <div className="text-yellow-400">
                      {'★'.repeat(r.score)} <span className="text-slate-400">{'★'.repeat(Math.max(0, 5 - r.score))}</span>
                    </div>
