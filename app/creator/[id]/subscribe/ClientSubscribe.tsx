// app/creator/[id]/subscribe/ClientSubscribe.tsx
'use client';
...
export default function ClientSubscribe({
  ...
  const doSubscribe = async (planId: bigint) => {
    try {
      ...
      <div>
        ...
        {plansLoading ? (
          ...
          <>
            {hasActiveSub !== null && (
              <div
                className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs border ${
                  hasActiveSub
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-slate-300">
                          {formatPrice(p.pricePerPeriod, p.token)} /{' '}
                          <PeriodBadge days={p.periodDays} />
                        </div>
                        {p.metadataURI ? (
                        ...
                        <div className="mt-2 text-xs text-slate-400">
                          Currency: {formatTokenLabel(p.token)}
                        </div>
                      </div>
      ...
      <div>
        ...
        {postsLoading ? (
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-2 text-sm text-slate-400">
              When the creator publishes paid posts, you’ll see them here.
            </div>
          </div>
        ) : (
          <ul className="grid gap-4 md:grid-cols-2">
