...
// ---- infer types from the hook so we don't need exported types
...
export default function ClientSubscribe({ creatorAddress }: { creatorAddress: `0x${string}` }) {
  const { address } = useAccount()
  const {
    // plans
    getCreatorPlanIds, getPlan, subscribe,
    // posts
    getCreatorPostIds, getPost, buyPost,
  } = useCreatorHub()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const out: Plan[] = []
        for (const id of ids) {
          const p = await getPlan(id) // ⬅️ renamed from readPlan
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const out: Post[] = []
        for (const id of ids) {
          const p = await getPost(id) // ⬅️ renamed from readPost
          if (p.active) out.push(p)
        }
        if (!alive) return
        setPostIds(ids)
