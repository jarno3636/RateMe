...
/** Ensure a KV row exists for this profile; bootstrap from chain if needed. */
async function resolveOrBootstrap(idOrHandleRaw: string): Promise<Creator | null> {
  ...
  // Bootstrap from chain
  let onchain: Creator | null = null
  if (isNumericId(key)) onchain = await fetchOnchainById(BigInt(key))
  ...
  // will set handle map + owner index
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    const idParam = lc(params.id || '')
    if (!idParam) return NextResponse.json({ ok: false, error: 'missing id' }, { status: 400 })
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // ensure KV exists
    const ensured = await resolveOrBootstrap(idParam)
