...
import { Star, Share2, Loader2, RefreshCw } from 'lucide-react';
...
export default function CreatorGrid() {
  const {
    ...
    isLoading,
    mutate, // for retry
  } = useSWR<ApiResponse>('/api/creators?limit=12', fetcher, {
  ...
  return (
    <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {rows.map((c) => {
        ...
        return (
          <li key={c.id} className="group flex flex-col rounded-2xl border border-white/10 bg-white/5 p-4 transition hover:border-white/20">
            {/* Top: avatar + names */}
            <div className="flex items-center gap-3">
            ...
            {c.ratingCount ? (
              <div className="mt-2 inline-flex items-center gap-1 text-sm text-slate-400" aria-label={`Rating ${c.avgRating?.toFixed(1)} from ${c.ratingCount} ratings`}>
                <Star className="h-3.5 w-3.5 text-yellow-400" aria-hidden />
                <span className="font-medium text-slate-200">{c.avgRating?.toFixed(1)}</span>
